...
function showContent(res){
    ...
    firstpara.className="predictPara";
    firstpara.innerHTML="";
    predict.appendChild(firstpara);

    var secondpara = document.createElement('p');
    secondpara.className="predictPara";
    secondpara.innerHTML="Tôi nghĩ chủ đề \"" + change[res.class] + "\" là chủ đề mà bạn muốn tìm! Nhấn vào đây để hiển thị các kết quả thuộc chủ đề này.";
    predict.appendChild(secondpara);

}

function showContentByClass(res){
    var table = document.getElementById('table__content');
    table.innerHTML="";
    var row; 
...
$("#watchButton").click(function(){
    ...
    if(text == ""){
        ...
        settings = {
            "async": true,
            "crossDomain": true,
            "url": "http://127.0.0.1:5000/searchAll",
            "method": "POST",
            "headers": {
...
$('#predict').click(function(){
    settings = {
        "async": true,
        "crossDomain": true,
        "url": "http://127.0.0.1:5000/searchOnlyClass",
        "method": "POST",
        "headers": {
            "Content-Type": "application/json",
            "Cache-Control": "no-cache",
